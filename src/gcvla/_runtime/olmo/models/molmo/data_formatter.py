# Modifications by Tommoro: public runtime wording adaptation.
...
from olmo.extra_tokens import get_robot_output_trigger_suffix
...
from olmo.preprocessing.point_formatter import PointFormattingV1, UnifiedPointFormatter, \
    PointFormatter
from olmo.preprocessing.box_formatter import BoxFormatter
from olmo.preprocessing.sequence_length_utils import MalformedExampleError
from olmo.util import parse_timestamp


def build_no_visible_initial_point_error(example: Dict[str, Any]) -> MalformedExampleError:
    metadata = example.get("metadata", {})
    if isinstance(metadata, dict):
        metadata = dict(metadata)
    else:
        metadata = {"raw_metadata": metadata}
    metadata.setdefault("style", example.get("style"))
    metadata.setdefault("label", example.get("label"))
    metadata.setdefault("point_frame_count", len(example.get("points") or []))
    video_timestamps = (example.get("video") or {}).get("timestamps")
    metadata.setdefault("video_timestamp_count", 0 if video_timestamps is None else len(video_timestamps))
    return MalformedExampleError(
        reason="no_visible_initial_point",
        details=(
            "No visible initial point found for single point tracking after "
            "filtering to video timestamps or in the original frames"
        ),
        metadata=metadata,
    )
...
GENERAL_PROMPTS_V1 = {
    ...
    "plain": ["{question}"],
    "3d_bbox_detection": ["{question}"],  # 3D object detection - prompt already contains full instructions
    "multiple_choice": [
    ...
    "detection": [
        "Detect {label}",
        "Detect all {label} in the image",
        "Find all {label} and draw bounding boxes",
        "Locate all {label} with bounding boxes",
        "Where are the {label}? Draw boxes around them.",
        "Show me bounding boxes for all {label}",
        "Draw boxes around each {label}",
        "Detect and localize {label}",
        "Find {label} and output their bounding boxes",
...
STYLE_TO_GENERAL_PROMPT = {
    ...
    "plain": "plain",
    "omni3d_3dod_vst": "3d_bbox_detection",  # 3D object detection (Omni3D, VST format)
}
...
GENERAL_PROMPTS_V1["robot_action"] = [
    "The task is to {task}. The setup is {setup_type}.{state_clause} The expected control mode is {control_mode}. Given these, what action should the robot take to complete the task?",
]
GENERAL_PROMPTS_V1["robot_depth"] = [
    "The task is to {task}. The setup is {setup_type}.{state_clause} The expected control mode is {control_mode}. Given these, what is the depth map of the main image?",
]
GENERAL_PROMPTS_V1["robot_depth_action"] = [
    "The task is to {task}. The setup is {setup_type}.{state_clause} The expected control mode is {control_mode}. Given these, first predict the depth map of the main image and then predict the action the robot should take to complete the task?",
]
...
class DataFormatter(BaseConfig):
    ...
    _box_formatter: Optional[BoxFormatter] = dataclasses.field(default=None, metadata={"omegaconf_ignore": True})
    ...
    def __post_init__(self):
        ...
        # Initialize box formatter for detection (use 1000 scale to match pointing)
        self._box_formatter = BoxFormatter(coordinate_scale="1000", sort_order="xy")
    ...
    def format_boxes(self, example):
        """Format bounding boxes for detection output."""
        ...
        if "label" in example:
            ...
            label = example.get("question", "object")
        if len(boxes) == 0:
            return "There are none."
        scale = example.get("box_scale", 100)
        return self._box_formatter.format_image_boxes(boxes, scale, label, mode="detection")

    def format_multi_points(self, example, rng):
        ...
        def point_count():
            ...
            if self.p_multi_point_all_image:
                if n_images == 1 or rng.random() < self.p_multi_point_all_image:
                    ...
                    selected_images = rng.choice(all_images, size=n_images_to_select, replace=False).tolist()
        ...
        def rank_by_cnt():
            ...
            if not image_rankings:
                ...
                if len(remaining_indices) >= n_additional:
                    additional_indices = rng.choice(remaining_indices, size=n_additional, replace=False).tolist()
                else:
        ...
        selected_category = rng.choice(category_names, p=weights)
        category_function = qa_categories[selected_category]

        question, answer = category_function()
        return question, answer
    ...
    def format_video_object_track_points(self, example, is_training=False, rng=None):
        ...
        if style == "video_point_track_per_frame":
            ...
            if not initial_points:
                ...
                if initial_point is None:
                    raise build_no_visible_initial_point_error(example)
    ...
    def get_system_prompt(self, style, for_inference, messages, is_training, rng):
        ...
        if self.system_prompt == "style":
            ...
            # style tag
            if style in DEMO_STYLES or style in VIDEO_MC_STYLES or style in IMAGE_MC_STYLES or style in ROBOT_STYLES:
                prefix = ""
            else:
                prefix = style + ":"
    ...
    def get_user_prompt(self, example, is_training=True, for_inference=False, rng=None):
        ...
        if "prompt" in example:
            ...
            if "prompt" in example:
                ...
                if "question" in example:
                    ...
                    prompt = example.get("label", "object").lower()
                output = self.format_boxes(example)
            ...
            if style in ROBOT_STYLES:
                ...
                if "question" in example:
                    ...
                    if "label" in example:
                        ...
                        label = "object"
                    prompt = apply_keyword_prompt(GENERAL_PROMPTS_V1["detection"], dict(example, label=label), rng, dbg=self.debug)
                output = self.format_boxes(example)
                ...
                prompt, output = self.format_multi_points(example, rng)
                metadata = example.get("metadata", {})
                metadata["answer"] = output
        ...
        prompt_only = bool(isinstance(example, dict) and example.get("prompt_only", False))
        if output is None and not for_inference and not prompt_only:
            if "answers" in example:
                ...
                print(example)
                raise ValueError("No output in example; inference targets are required")
    ...
    def _format_example(self, message, example, is_training, for_inference, rng):
        ...
        if isinstance(message, str):
            ...
            if "multi_turn_messages" in example:
                ...
                prompt_only = bool(message.get("prompt_only", False))
                if not for_inference and not prompt_only:
                    assert response is not None
                    messages = [prompt, response]
                else:
                    messages = [prompt]
        ...
        trigger_suffix = get_robot_output_trigger_suffix(style)
        if trigger_suffix and messages:
            trigger_ix = len(messages) - 1 if len(messages) % 2 == 1 else len(messages) - 2
            messages[trigger_ix] = messages[trigger_ix] + trigger_suffix
