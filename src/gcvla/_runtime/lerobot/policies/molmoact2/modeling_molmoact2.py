...
def _resolve_molmoact2_root() -> Path:
    ...
    for parent in Path(__file__).resolve().parents:
        if parent.name == "lerobot":
            ...
            if (repo_root / "olmo").is_dir() :
                return repo_root
        if (parent / "olmo").is_dir() :
            return parent
...
def _ordered_camera_images(obs: Dict[str, Any]) -> tuple[List[np.ndarray], List[str]]:
    """Resolve runtime camera names into the fixed head/left/right slot order."""
    entries: dict[str, Any] = {
        str(key): value for key, value in obs.items() if str(key).startswith("observation.images")
    }
    if not entries:
        fallback = obs.get("images", obs.get("image"))
        images: List[np.ndarray] = []
        _append_images(images, fallback)
        return images, ["fallback"] * len(images)
    ...
    slot_sources = {
        ...
        "right_wrist": (
            ...
            "observation.images.right_eye_in_hand_image",
        ),
    }
    images: List[np.ndarray] = []
    names: List[str] = []
    used: set[str] = set()
    for slot, candidates in slot_sources.items():
        source = next((key for key in candidates if key in entries), None)
        if source is None:
            continue
        before = len(images)
        _append_images(images, entries[source])
        names.extend([slot] * (len(images) - before))
        used.add(source)
    # Preserve unknown views only when no canonical slot matched them.
    if not images:
        for key, value in entries.items():
            before = len(images)
            _append_images(images, value)
            names.extend([key] * (len(images) - before))
    return images, names


def _zero_pad_missing_camera_views(images: List[np.ndarray]) -> None:
    """Make eval camera layout explicit when a checkpoint expects three views."""
    if not _env_flag("MOLMOACT2_ZERO_PAD_MISSING_CAMERAS"):
        return
    expected = int(os.getenv("MOLMOACT2_EXPECTED_CAMERA_COUNT", "3"))
    if expected < 1 or len(images) >= expected:
        return
    reference = images[0]
    original_count = len(images)
    images.extend(np.zeros_like(reference) for _ in range(expected - len(images)))
    print(
        f"[MolmoAct2] zero-padded camera views: {original_count}->{len(images)} "
        f"(expected={expected})",
        flush=True,
    )
...
def _build_example(
    ...
    *,
    ...
    if inference_action_mode == "continuous" and not _env_flag(
        "MOLMOACT2_INFERENCE_USE_TRAIN_PROMPT_FIELDS"
    ):
        ...
        example["answers"] = (
            DEPTH_START_TOKEN
            if _env_flag("MOLMOACT2_INFERENCE_PREFILL_DEPTH_START")
            and style_uses_depth_output(style)
            else ""
        )
    return example
...
class MolmoAct2Policy(PreTrainedPolicy):
    ...
    def __init__(self, config: MolmoAct2Config, *inputs, **kwargs):
        ...
        self._episode_flow_generators: Dict[tuple[int, int], torch.Generator] = {}
        self._debug_action_log_count = 0
        self._debug_contract_log_count = 0
        self._debug_depth_generation_count = 0
        self._prepare_model()
    ...
    @staticmethod
    def _resolve_inference_dtype(config: Any, device: torch.device) -> torch.dtype:
        requested = os.environ.get("MOLMOACT2_INFERENCE_DTYPE") or getattr(config, "inference_dtype", "float32")
        requested = str(requested or "auto").strip().lower()

        if device.type != "cuda":
            if requested not in {"auto", "float32", "fp32", "full"}:
                log.warning("Forcing float32 inference dtype on non-CUDA device %s.", device)
        ...
        if requested in {"float32", "fp32", "full"}:
            return torch.float32
        raise ValueError(
            f"Unsupported inference_dtype={requested!r}. "
            "Expected auto, bfloat16/bf16, float16/fp16/half, or float32/fp32/full."
        )
    ...
    def _prepare_model(self) -> None:
        ...
        from gcvla.weight_loader import load_model_state
        ...
        inference_dtype = self._resolve_inference_dtype(cfg, device)
        ...
        model.eval()
        log.info("Loaded local MolmoAct2 checkpoint from %s on %s with dtype=%s", checkpoint_dir, device, inference_dtype)
        ...
        if cfg.inference_action_mode == "discrete":
            action_start_token_id, action_end_token_id = resolve_family_boundary_token_ids(
                tokenizer,
                ACTION_TOKENS,
                "FAST/action-first inference requires single-token <action_start>/<action_end>.",
            )
        ...
        if style_uses_depth_output(default_style):
            ...
            if not has_depth_output or (
                cfg.inference_action_mode == "discrete" and action_format not in {"discrete", "both"}
            ):
                raise ValueError(
                    f"Style '{default_style}' requires a checkpoint with depth tokens"
                    " and discrete/both action_format for discrete inference."
                )
    ...
    def reset(self) -> None:
        ...
        if hf_backend is not None:
            hf_backend.reset()
            # The HF backend clears queues, while the loaded model owns the
            # episode-local GCRF/router state.
            model = getattr(hf_backend, "model", None)
            if model is not None and hasattr(model, "reset"):
                model.reset()
            action_expert = None if model is None else getattr(model, "action_expert", None)
            if action_expert is not None and hasattr(
                action_expert, "reset_gcvla_temporal_geometry"
            ):
                action_expert.reset_gcvla_temporal_geometry()
            return
        ...
        self._last_model_inference_s = 0.0
        self._last_model_inference_calls = 0
        self._episode_flow_generators = {}
        handles = self._handles
        # Direct-checkpoint inference does not go through hf_backend.reset().
        # Reset model-owned GCRF/router state here so a reused B5 slot does not
        # carry its causal prefix into the next batch of episodes.
        model = None if handles is None else getattr(handles, "model", None)
        if model is not None and hasattr(model, "reset"):
            model.reset()
        action_expert = None if handles is None else getattr(handles.model, "action_expert", None)
        if action_expert is not None and hasattr(action_expert, "reset_gcvla_temporal_geometry"):
            action_expert.reset_gcvla_temporal_geometry()

    def _episode_flow_generator(
        self,
        *,
        batch_idx: int,
        device: torch.device,
        fallback: Optional[torch.Generator],
    ) -> Optional[torch.Generator]:
        """Return RNG owned by one canonical episode, never by process history."""
        if not _env_flag("MOLMOACT2_EPISODE_FLOW_RNG"):
            return fallback
        metadata = getattr(self, "_gcrf_trace_metadata", {}) or {}
        episode_seeds = metadata.get("episode_seeds") or []
        if not 0 <= batch_idx < len(episode_seeds):
            raise RuntimeError(
                "MOLMOACT2_EPISODE_FLOW_RNG requires evaluator episode_seeds "
                f"for slot {batch_idx}; refusing global CUDA RNG fallback"
            )
        episode_seed = int(episode_seeds[batch_idx])
        key = (episode_seed, int(batch_idx))
        generator = self._episode_flow_generators.get(key)
        if generator is None:
            generator = torch.Generator(device=device)
            generator.manual_seed(episode_seed)
            self._episode_flow_generators[key] = generator
        return generator
    ...
    def _call_generate_inference_result(self, *args, **kwargs) -> tuple[MolmoAct2InferenceResult, float]:
        ...
        if handles is not None and handles.device.type == "cuda":
            try:
    ...
    def get_optim_params(self) -> dict:
        ...
        raise NotImplementedError("Unsupported policy operation.")
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], **kwargs) -> Tensor:
        ...
        for idx in range(batch_size):
            ...
            flow_generator = self._episode_flow_generator(
                batch_idx=idx,
                device=handles.device,
                fallback=requested_generator,
            )
    ...
    def _obs_to_example(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        images, image_slot_names = _ordered_camera_images(obs)
        raw_image_keys = [str(key) for key in obs if str(key).startswith("observation.images")]
        if not images:
            raise ValueError("No image data found in observation for MolmoAct2.")
        raw_image_count = len(images)
        _zero_pad_missing_camera_views(images)
        if _env_flag("MOLMOACT2_LOG_CAMERA_CONTRACT") and self._debug_contract_log_count < 20:
            valid_mask = [True] * raw_image_count + [False] * (len(images) - raw_image_count)
            print(
                "[MolmoAct2] camera_contract "
                f"raw_keys={raw_image_keys} assigned_slots={image_slot_names} raw_count={raw_image_count} "
                f"slot_count={len(images)} valid_mask={valid_mask}",
                flush=True,
            )
            self._debug_contract_log_count += 1
    ...
    def _resolve_style_for_inference(handles: _MolmoHandles) -> str:
        if _env_flag("MOLMOACT2_FORCE_ROBOT_ACTION_STYLE"):
            return "robot_action"
    ...
    def _drop_depth_tokens_from_action_expert_mask(
        ...
        if _env_flag("MOLMOACT2_GCVLA_FIXED_QUERY_DEPTH"):
            # Fixed query slots never contain teacher-forced depth values and are safe KV.
            return encoder_attention_mask
        if not _env_flag("MOLMOACT2_ACTION_EXPERT_DROP_DEPTH_TOKENS"):
            return encoder_attention_mask
        depth_token_ids = set(handles.depth_token_id_to_bin.keys())
        if handles.depth_start_token_id is not None:
            depth_token_ids.add(int(handles.depth_start_token_id))
        if handles.depth_end_token_id is not None:
            depth_token_ids.add(int(handles.depth_end_token_id))
        if not depth_token_ids:
            return encoder_attention_mask
        ids = torch.as_tensor(sorted(depth_token_ids), device=full_input_ids.device, dtype=full_input_ids.dtype)
        depth_mask = (full_input_ids.unsqueeze(-1) == ids).any(dim=-1)
    ...
    def _decode_discrete_depth_bins(
        ...
        if _env_flag("MOLMOACT2_LOG_DEPTH_GENERATION") and self._debug_depth_generation_count < 5:
            known_depth_ids = [
                token_id for token_id in flat_generated_ids if token_id in handles.depth_token_id_to_bin
            ]
            print(
                "[MolmoAct2] depth_generation "
                f"shape={tuple(generated_token_ids.shape)} "
                f"generated_len={len(flat_generated_ids)} "
                f"known_depth_count={len(known_depth_ids)} "
                f"first_ids={flat_generated_ids[:64]} "
                f"decoded={_format_log_text(_decode_token_ids(handles.tokenizer, flat_generated_ids[:64]))}",
                flush=True,
            )
            self._debug_depth_generation_count += 1
        if not depth_token_bins:
            if _env_flag("MOLMOACT2_DEPTH_DECODE_FALLBACK"):
                fallback_len = int(getattr(handles, "num_depth_codes", 1) or 1)
                print(
                    "WARNING: Model generated no decodable depth tokens between "
                    "<depth_start>/<depth_end>; using zero fallback bins for eval continuity.",
                    flush=True,
                )
                depth_token_bins = [0] * fallback_len
            else:
                raise RuntimeError(
    ...
    def _generate_depth_conditioned_actions(
        ...
        *,
        ...
        generation_batch = self._build_generation_batch(collated)


        depth_generation_steps = _DISCRETE_GENERATION_MAX_STEPS
        if _env_flag("MOLMOACT2_GCVLA_FIXED_QUERY_DEPTH"):
            zero_depth_ids = [
                token_id
                for token_id, depth_bin in handles.depth_token_id_to_bin.items()
                if int(depth_bin) == 0
            ]
            if len(zero_depth_ids) != 1:
                raise RuntimeError(
                    "Expected exactly one depth token for the fixed GC query embedding, got "
                    f"{len(zero_depth_ids)}."
                )
            query_count = int(handles.num_depth_codes)
            generated_token_ids = torch.full(
                (generation_batch["input_ids"].shape[0], query_count + 2),
                int(zero_depth_ids[0]),
                device=generation_batch["input_ids"].device,
                dtype=generation_batch["input_ids"].dtype,
            )
            generated_token_ids[:, 0] = int(handles.depth_start_token_id)
            generated_token_ids[:, -1] = int(handles.depth_end_token_id)
            full_input_ids = torch.cat(
                [generation_batch["input_ids"], generated_token_ids], dim=1
            )
            query_batch = dict(generation_batch)
            query_batch["input_ids"] = full_input_ids
            attention_mask = query_batch.get("attention_mask")
            if torch.is_tensor(attention_mask) and attention_mask.ndim == 2:
                query_batch["attention_mask"] = torch.cat(
                    [
                        attention_mask,
                        torch.ones_like(attention_mask[:, :1]).expand(
                            -1, generated_token_ids.shape[1]
                        ),
                    ],
                    dim=1,
                )
            query_output = handles.model(**query_batch, use_cache=True)
            encoder_kv_states = query_output.attn_key_values
            depth_bins = torch.zeros(
                (generated_token_ids.shape[0], query_count),
                device=handles.device,
                dtype=torch.long,
            )
        elif _env_flag("MOLMOACT2_INFERENCE_FORCE_LEGACY_ZERO_DEPTH_SEQUENCE"):
            zero_depth_ids = [
                token_id
                for token_id, depth_bin in handles.depth_token_id_to_bin.items()
                if int(depth_bin) == 0
            ...
            if len(zero_depth_ids) != 1:
                raise RuntimeError(
                    "Expected exactly one depth token for bin 0, got "
                    f"{len(zero_depth_ids)}."
                )
            generated_token_ids = torch.full(
                (generation_batch["input_ids"].shape[0], _DISCRETE_GENERATION_MAX_STEPS),
                int(zero_depth_ids[0]),
                device=generation_batch["input_ids"].device,
                dtype=generation_batch["input_ids"].dtype,
            )
            ...
            forced_batch = dict(generation_batch)
            forced_batch["input_ids"] = full_input_ids
            attention_mask = forced_batch.get("attention_mask")
            if torch.is_tensor(attention_mask) and attention_mask.ndim == 2:
                forced_batch["attention_mask"] = torch.cat(
                    [
                        attention_mask,
                        torch.ones_like(attention_mask[:, :1]).expand(
                            -1, generated_token_ids.shape[1]
            ...
            forced_output = handles.model(**forced_batch, use_cache=True)
            encoder_kv_states = forced_output.attn_key_values
            depth_bins = torch.zeros(
                (generated_token_ids.shape[0], generated_token_ids.shape[1] - 1),
                device=handles.device,
                dtype=torch.long,
            )
        else:
            generated = handles.model.generate(
                generation_batch,
                max_steps=depth_generation_steps,
                beam_size=1,
        ...
        if encoder_kv_states is None:
            raise RuntimeError("Depth conditioning did not return KV cache.")
        ...
        encoder_attention_mask = self._drop_depth_tokens_from_action_expert_mask(
            full_input_ids,
        ...
        return action_chunk, generated_token_ids[:, None, :], depth_bins
    ...
    def select_action(self, batch: dict[str, Tensor], **kwargs) -> Tensor:
        ...
        for idx in range(batch_size):
            if hasattr(handles, "model"):
                handles.model._gcrf_router_stream_index = idx
            obs_slice = self._slice_observation_batch(batch, idx, batch_size)
            history = self._update_obs_history(idx, obs_slice, handles.n_obs_steps)
            ...
            if not action_queue:
                ...
                style = self._resolve_style_for_inference(handles)
                collated = self._collate_example(combined_example, handles)
                # MolmoAct2 inference is performed one vector-env slot at a
                # time; keep router history keyed by that stable slot index.
                if hasattr(handles, "model"):
                    handles.model._gcrf_router_stream_index = idx
                    action_expert = getattr(handles.model, "action_expert", None)
                    if action_expert is not None:
                        action_expert._gcvla_temporal_geometry_stream_index = idx
                result, inference_elapsed = self._call_generate_inference_result(
                    collated,
                    handles,
                    norm_tag=requested_norm_tag,
                ...
                # Record replan indices for reproducible action traces.
                replan_counts = getattr(self, "_gcvla_replan_counts", None)
                if replan_counts is None:
                    replan_counts = defaultdict(int)
                    self._gcvla_replan_counts = replan_counts
                replan_counts[idx] += 1
                self._maybe_log_generated_text(
                    collated.get("input_ids"),
                    result.generated_token_ids,
            ...
            action_trace_path = os.environ.get("MOLMOACT2_ACTION_TRACE_PATH", "").strip()
            if action_trace_path:
                trace_steps = getattr(self, "_gcvla_action_trace_steps", None)
                if trace_steps is None:
                    trace_steps = defaultdict(int)
                    self._gcvla_action_trace_steps = trace_steps
                trace_path = Path(action_trace_path)
                trace_path.parent.mkdir(parents=True, exist_ok=True)
                trace_provenance = getattr(self, "_gcvla_trace_provenance", {})
                trace_init_indices = trace_provenance.get("init_indices", [])
                trace_episode_seeds = trace_provenance.get("episode_seeds", [])
                trace_row = {
                    "batch_index": idx,
                    "step": trace_steps[idx],
                    "replan_index": int(getattr(self, "_gcvla_replan_counts", {}).get(idx, 0)),
                    "action": next_action.detach().float().cpu().reshape(-1).tolist(),
                }
                if idx < len(trace_init_indices):
                    trace_row["init_index"] = int(trace_init_indices[idx])
                if idx < len(trace_episode_seeds):
                    trace_row["episode_seed"] = int(trace_episode_seeds[idx])
                gc_bins = getattr(self, "_last_depth_video_codes_by_batch", {}).get(idx)
                if gc_bins is not None:
                    trace_row["gc_depth_bins"] = gc_bins.tolist()
                with trace_path.open("a") as trace_handle:
                    trace_handle.write(json.dumps(trace_row) + "\n")
                trace_steps[idx] += 1
        ...
        if _env_flag("MOLMOACT2_LOG_NORMALIZED_ACTION_STATS") and self._debug_action_log_count < 20:
            values = action_tensor.detach().float().cpu().numpy()
            print(
                "[MolmoAct2] normalized_action_stats "
                f"shape={values.shape} min={values.min(axis=0).tolist()} "
                f"max={values.max(axis=0).tolist()} mean={values.mean(axis=0).tolist()}",
                flush=True,
            )
        if handles.robot_processor is not None:
            ...
            normalized_gripper = action_tensor.detach().float().cpu().numpy()[:, -1].tolist()
            action_tensor = handles.robot_processor.unnormalize_action(
                action_tensor,
                repo_id=requested_norm_tag,
            )
            # EEF gripper targets are already in the physical [-1, 1] space when
            # normalize_gripper=false; do not apply pose statistics to them.
            robot_meta = getattr(handles.robot_processor, "metadata_by_tag", {}).get(resolved_tag, {})
            preserve_raw_gripper = bool(robot_meta.get("normalize_gripper") is False) or _env_flag(
                "MOLMOACT2_PRESERVE_RAW_GRIPPER"
            )
            if preserve_raw_gripper and int(expected_action_dim or 0) in {7, 8}:
                action_tensor[..., int(expected_action_dim) - 1] = torch.as_tensor(
                    normalized_gripper,
                    device=action_tensor.device,
                    dtype=action_tensor.dtype,
                )
            if _env_flag("MOLMOACT2_LOG_ACTION_CONTRACT") and self._debug_action_log_count < 20:
                physical = action_tensor.detach().float().cpu().numpy()[:, -1].tolist()
                print(
                    "[MolmoAct2] action_contract "
                    f"requested_norm_tag={requested_norm_tag!r} resolved_tag={resolved_tag!r} "
                    f"expected_dim={expected_action_dim} shape={tuple(action_tensor.shape)} "
                    f"gripper_normalized={normalized_gripper} gripper_physical={physical}",
                    flush=True,
                )
        if _env_flag("MOLMOACT2_LOG_ACTION_STATS") and self._debug_action_log_count < 20:
            values = action_tensor.detach().float().cpu().numpy()
            print(
                "[MolmoAct2] action_stats "
                f"shape={values.shape} min={values.min(axis=0).tolist()} "
                f"max={values.max(axis=0).tolist()} mean={values.mean(axis=0).tolist()}",
                flush=True,
            )
            self._debug_action_log_count += 1
