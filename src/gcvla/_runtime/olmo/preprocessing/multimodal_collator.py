...
class MMCollator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        def _stack_optional_dense(
            ...
            if not any(present):
                return
            if not all(present) and missing_fill is None:
            ...
            if not all(present):
                ...
                for value in values:
                    if value is None:
                        arr = np.full(first_shape, missing_fill, dtype=numpy_dtype)
                    else:
                        arr = np.asarray(value)
                        if numpy_dtype is not None:
                            arr = arr.astype(numpy_dtype, copy=False)
        ...
        if packed_depth_tensors is not None:
            ...
            _stack_optional_dense("depth_updated_mask", torch.bool, numpy_dtype=np.bool_, missing_fill=False)
            _stack_optional_dense("depth_buffer_codes", torch.long, numpy_dtype=np.int64, missing_fill=-1)
            _stack_optional_dense(
                "hbz_wrist_geometry_target",
                torch.float32,
                numpy_dtype=np.float32,
                missing_fill=0.0,
            )
            _stack_optional_dense(
                "hbz_wrist_geometry_target_is_valid",
                torch.bool,
