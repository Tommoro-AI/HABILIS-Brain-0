...
class _FeatureNormalizer:
    ...
    def normalize(self, x):
        ...
        if stats_ref is None:
            stats_ref = self.min_val
        if stats_ref is None:
            stats_ref = self.q_low
        if (
            stats_ref is not None
            and getattr(arr, "ndim", 0) >= 1
            and getattr(stats_ref, "ndim", 0) == 1
            and arr.shape[-1] > stats_ref.shape[-1]
        ):
            # Some schema-normalized caches store actions already padded to the
            # model max action dim. Dataset statistics are still computed over
            # the active prefix. Normalize only that prefix and keep padded
            # dimensions unchanged; action_dim_is_pad masks them from loss.
            prefix_dim = int(stats_ref.shape[-1])
            out = np.array(arr, copy=True)
            prefix = arr[..., :prefix_dim]
            normalized_prefix = _normalize_array(
                prefix,
                mean=self.mean,
                std=self.std,
            ...
            if _uses_bounded_normalized_range(self.mode):
                normalized_prefix = np.clip(normalized_prefix, -1.0, 1.0)
            if self.mask is not None:
                normalized_prefix = np.where(self.mask, normalized_prefix, prefix)
            if self.zero_mask is not None:
                normalized_prefix = np.where(self.zero_mask, 0.0, normalized_prefix)
            out[..., :prefix_dim] = normalized_prefix
            if torch.is_tensor(x):
    ...
    def unnormalize(self, x):
        ...
        if (
            ...
            prefix = arr[..., :prefix_dim]
            if _uses_bounded_normalized_range(self.mode):
                prefix = np.clip(prefix, -1.0, 1.0)
            unnormalized_prefix = _unnormalize_array(
                prefix,
                mean=self.mean,
                std=self.std,
                min_val=self.min_val,
            ...
            if self.mask is not None:
                unnormalized_prefix = np.where(self.mask, unnormalized_prefix, prefix)
            out[..., :prefix_dim] = unnormalized_prefix
            if torch.is_tensor(x):
                return torch.as_tensor(out, device=x.device, dtype=x.dtype)
            return out
        if _uses_bounded_normalized_range(self.mode):
